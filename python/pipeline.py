...
if __name__ == "__main__":
    ...
    print(f"Total Return: {cpp_output['analytics']['total_return']:.2%}")
    print(f"CAGR: {cpp_output['analytics']['cagr']:.2%}")
    print(f"Annualized Volatility: {cpp_output['analytics']['annualized_volatility']:.2%}")
    print(f"Sharpe Ratio: {cpp_output['analytics']['sharpe_ratio']:.4f}")
    print(f"Max Drawdown: {cpp_output['analytics']['max_drawdown']:.2%}")
    print("\nReasoning:")
    for line in cpp_output["reasoning"]:
        print(f"  {line}")
